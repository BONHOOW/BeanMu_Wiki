...
JSON 스키마는 프롬프트의 📦 섹션이 기준이고, 파서는 `BeanMuWiki/Models.swift`의 `BeanImport`입니다. 실제 출력 예시는 `docs/import-example.json`에 있으며, 그대로 복사해 가져오기로 테스트할 수 있습니다. 프롬프트는 하리오 V60 + 홀츠클로츠 E80 그라인더 기준으로 쓰여 있으니 장비가 다르면 "고정 장비 세팅" 부분만 고쳐 쓰면 됩니다.
